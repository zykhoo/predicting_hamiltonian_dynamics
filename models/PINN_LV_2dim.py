...
def lossfuc(model,mat,x,y,device,x0,H0,dim,c1=1,c2=10,c3=1,c4=1,verbose=False):
    ...
    qprime=(mat[:,2])
    pprime=(mat[:,3])
    f1=torch.mean((p*q*dHdp+qprime)**2,dim=0)
    f2=torch.mean((p*q*dHdq-pprime)**2,dim=0)   
...
def get_grad(model, z,device):
		...
		return -z[0][0]*z[1][0]*dH.detach().cpu().numpy()[1], z[0][0]*z[1][0]*dH.detach().cpu().numpy()[0] # negative dH/dp is dq/dt for LV
