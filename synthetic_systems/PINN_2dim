...
def classicIntNNH_autograd(z,h,model):
	## classical symplectic Euler scheme
		...
		q=z[:dim]
		p=z[dim:]		
		fstage = lambda stg: h * get_grad(model, z)[0]
...
def classicTrajectoryNNH_autograd(z,h,model,N=1):
  ...
  trj[:,0] = z.copy()
  if N == 1:
    return z.reshape(-1,1), classicIntNNH_autograd(trj[:,0].reshape(-1,1),h,model).reshape(-1,1)
  else:
    for j in range(0,N+1):
      trj[:,j+1] = classicIntNNH_autograd(trj[:,j].reshape(-1,1).copy(),h,model)
  return trj[:, :-1], trj[:, 1:]
